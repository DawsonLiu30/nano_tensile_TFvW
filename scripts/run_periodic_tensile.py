...
def _collect_stress_metrics(atoms, stress, area_ref_ellipse: float) -> dict[str, float]:
    ...
    wire_stress_current = _stress_from_cell_sigma(
        ...
        wire_area_xy=area_cur_ellipse,
    )
    wire_stress_ref = _stress_from_cell_sigma(
        sigma_cell_zz,
        cell_area_xy=cell_area_xy,
        wire_area_xy=area_ref_ellipse,
    )
    return {
...
def _plot_periodic_summary(summary_csv: Path, out_png: Path) -> None:
    ...
    stress_key = _pick_summary_column(
        field_names,
        ["wire_stress_ref_GPa", "wire_stress_current_GPa", "sigma_cell_zz_GPa"],
    )
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Run quasistatic periodic tensile loading for paper-style nanowires.")
...
def main() -> None:
    ...
    print(
        f"[init] L0={lz_ref:.6f} A area_ref_ellipse={area_ref_ellipse:.6f} A^2 "
    ...
    if resume_results is None:
        summary.write_text(
            "cycle,strain,energy_eV,sigma_cell_zz_GPa,wire_stress_current_GPa,wire_stress_ref_GPa,"
            "wire_area_ref_ellipse_A2,wire_area_current_ellipse_A2,wire_area_current_bbox_A2,"
            "cell_area_xy_A2,wire_span_x_A,wire_span_y_A,n_atoms\n",
            encoding="utf-8",
        )
        stress_metrics0 = _collect_stress_metrics(atoms_eq, stress=stress0, area_ref_ellipse=area_ref_ellipse)
        _append_summary_row(
            summary,
            {
                "cycle": 0,
    ...
    for cyc in range(start_cycle, int(args.cycles) + 1):
        ...
        stress_metrics = _collect_stress_metrics(atoms_rlx, stress=stress, area_ref_ellipse=area_ref_ellipse)
        ...
        print(
            ...
            f"wire_stress_ref={row['wire_stress_ref_GPa']:+.6f} GPa"
        )
