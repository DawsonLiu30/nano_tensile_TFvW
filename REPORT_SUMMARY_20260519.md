...
### Periodic Tensile Stress Definition

- All nanocolumn and nanocrystal tensile models are infinite along z through
  axial periodic boundary conditions.
- The z cell length is a periodic repeat/supercell length, not a finite
  physical column length.
- The formal tensile stress is the area-corrected axial wire Cauchy stress:

```text
cauchy_wire_zz_GPa = sigma_cell_zz_GPa * A_cell / A_wire,current
```

- Raw simulation-cell stress is not directly used as the wire stress because
  the x-y supercell contains vacuum and would dilute the stress.
- `finite-grip`, `grip-reaction`, `top grip`, and `bottom grip` are excluded
  from the formal thesis terminology.
...
7. Nanocolumn and nanocrystal tensile models are axially periodic infinite
   structures; finite-grip terminology is excluded from the formal workflow.
